...
CREATE TRIGGER trg_turmas_upd BEFORE UPDATE ON turmas
  FOR EACH ROW EXECUTE FUNCTION set_atualizado_em();

CREATE TRIGGER trg_alunos_upd BEFORE UPDATE ON alunos
  FOR EACH ROW EXECUTE FUNCTION set_atualizado_em();

CREATE TRIGGER trg_materiais_upd BEFORE UPDATE ON materiais
  FOR EACH ROW EXECUTE FUNCTION set_atualizado_em();
...
CREATE OR REPLACE VIEW vw_distribuicao_nivel AS
SELECT
  nivel,
  COUNT(*) AS total,
...
-- Política: professor (service_role) vê tudo — padrão Supabase
-- Política: aluno autenticado vê apenas seu próprio registro
-- (Adapte com Supabase Auth conforme necessário)

-- Exemplo de política para alunos lerem seus próprios dados:
-- CREATE POLICY "aluno_ver_proprio" ON alunos
--   FOR SELECT USING (codigo_acesso = current_user);
